...
dae::CoopIntroScene::CoopIntroScene() : Scene("CoopIntro") {
}

void dae::CoopIntroScene::OnSetup() {
	auto instructionsTitleObject{ std::make_unique<GameObject>(Transform((640 / 2) - 220, 50)) };
	instructionsTitleObject->AddComponent<TextComponent>("INSTRUCTIONS - CO-OP", Qbert::GetInstance().GetFontMedium());
	...
	descriptionObject->AddComponent<ParagraphComponent>(
			"Co-op follows the same rules as Solo, but if\n"
			"one player loses all their lives, both lose.\n"
			"(P1's controls are also the exact same)",
			Qbert::GetInstance().GetFontVerySmall()
	);

	auto wasdObject{ std::make_unique<GameObject>(Transform(50, 250)) };
	wasdObject->AddComponent<TextureComponent>("P2 QBert Controls.png", 0.6f);
...
void dae::CoopIntroScene::OnEnter() {
	std::unique_ptr<Scene> levelDisplayScene{ std::make_unique<LevelDisplayScene>(LevelType::Coop) };
